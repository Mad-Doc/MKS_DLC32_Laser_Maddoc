...
namespace Spindles {
    ...
    uint32_t PWM::set_rpm(uint32_t rpm) {
        ...
        // In laser mode, keep EN pin off and drive only PWM output.
        if (laser_mode->get()) {
        ...
        // For laser engraving, apply PWM immediately to avoid line-to-line stutter.
        if (laser_mode->get()) {
            set_output(pwm_value);
            return 0;
        }
    ...
    void PWM::set_state(SpindleState state, uint32_t rpm) {
        ...
        if (state == SpindleState::Disable) {  // Halt or set spindle direction and rpm.
            ...
            set_rpm(rpm);
            // Laser mode must not energize spindle enable output.
            if (laser_mode->get()) {
                set_enable_pin(false);
            } else {
