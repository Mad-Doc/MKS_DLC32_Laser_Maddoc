...
static uint8_t get_event(lv_obj_t* obj) {
    if (obj == ready_src.ready_imgbtn_adjust_laser)      return ID_R_ADJUST;
...
static void disp_imgbtn(void) {
    ...
    ready_src.ready_imgbtn_Tool = lv_imgbtn_creat_mks(mks_global.mks_src_1, ready_src.ready_imgbtn_Tool, &png_tool_pre, &Tool, LV_ALIGN_IN_TOP_LEFT, 347, 10, event_handler);
}
...
static void disp_label(void) {
    ...
    label_for_imgbtn_name(mks_global.mks_src_1, ready_src.ready_label_adjust_laser, ready_src.ready_imgbtn_adjust_laser, 0, 0, mc_language.adjust);
    label_for_imgbtn_name(mks_global.mks_src_1, ready_src.ready_label_adjust_laser, ready_src.ready_imgbtn_adjust_laser, 0, 14, mc_language.adjust_laser);
    ...
    ready_src.ready_label_m_zpos = label_for_text(mks_global.mks_src,   ready_src.ready_label_m_zpos, NULL , 280, 166, LV_ALIGN_IN_TOP_LEFT, "Z:0");
        ...
        // ready_src.ready_label_wifi_status = label_for_btn_name(ready_src.ready_btn_wifi, ready_src.ready_label_wifi_status, 0, 0, mc_language.wifi_disconnect);
    #endif
}
