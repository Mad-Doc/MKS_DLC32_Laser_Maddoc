...
static void set_language_pack_en() {
	mc_language.back = BACK_EN;
	mc_language.yes = YES_EN;
	mc_language.no = NO_EN;

	mc_language.adjust_laser = ADJUSTEMENT_EN;
	...
	mc_language.grbl_settings_title = GRBL_SETTINGS_TITLE_EN;
	mc_language.preview_title = PREVIEW_TITLE_EN;
	mc_language.preview_wait = PREVIEW_WAIT_EN;
	mc_language.preview_close = PREVIEW_CLOSE_EN;
	mc_language.preview_err_no_laser = PREVIEW_ERR_NO_LASER_EN;
	mc_language.preview_err_memory = PREVIEW_ERR_MEMORY_EN;
	mc_language.preview_err_sd = PREVIEW_ERR_SD_EN;
	mc_language.preview_err_open = PREVIEW_ERR_OPEN_EN;
	mc_language.preview_err_render = PREVIEW_ERR_RENDER_EN;
}

static void set_language_pack_fr() {
	mc_language.back = BACK_FR;
	mc_language.yes = YES_FR;
	mc_language.no = NO_FR;

	mc_language.adjust_laser = ADJUSTEMENT_FR;
	mc_language.control = CONTROL_FR;
	mc_language.sculpture = SCULPTURE_FR;
	mc_language.tool = TOOL_FR;
	mc_language.Mpos = MPOS_FR;
	mc_language.Wpos = WPOS_FR;
	mc_language.wifi_connect = WIFI_CONNECT_FR;
	mc_language.wifi_disconnect = WIFI_DISCONNECT_FR;

	mc_language.xy_clear = XY_CLEAR_FR;
	mc_language.z_clear = Z_CLEAR_FR;
	mc_language.knife = KNIFE_FR;
	mc_language.next = NEXT_FR;
	mc_language.up = UP_FR;
	mc_language.cooling = COOLING_FR;
	mc_language.position = POSITION_FR;
	mc_language.speed_high = SPEED_HIGH_FR;
	mc_language.speed_mid = SPEED_MID_FR;
	mc_language.speed_low = SPEED_LOW_FR;
	mc_language.spindle = SPINDLE_FR;
	mc_language.carve = CARVE_FR;

	mc_language.dis_no_sd_card = DIS_NO_SDCARD_FR;

	mc_language.hold = HOLD_FR;
	mc_language.cycle = CYCLE_FR;
	mc_language.stop = STOP_FR;
	mc_language.adjust = ADJUST_FR;

	mc_language.dis_stop_print = DIS_STOP_CARVE_FR;
	mc_language.dis_homing = DIS_HOMEING_FR;
	mc_language.dis_no_hard_homing = DIS_NO_HARD_HOME_FR;
	mc_language.dis_homing_succeed = DIS_HOME_SUCCEED_FR;
	mc_language.dis_homing_fail = DIS_HOME_FAIL_FR;
	mc_language.dis_probe_set = DIS_PROBE_SET_FR;
	mc_language.dis_probe_succeed = DIS_PROBE_SECCEED_FR;
	mc_language.dis_probe_fail = DIS_PROBE_FAIL_FR;
	mc_language.dis_hard_limit = DIS_HARD_LIMIT_FR;
	mc_language.dis_soft_limit = DIS_SOFT_LIMIT_FR;
	mc_language.dis_unlock = DIS_UNLOCK_FR;
	mc_language.dis_wait_mc_stop = DIS_WAIT_MC_STOP_FR;
	mc_language.dis_pos_succeed = DIS_POS_SUCCEED_FR;
	mc_language.grbl_settings_title = GRBL_SETTINGS_TITLE_FR;
	mc_language.preview_title = PREVIEW_TITLE_FR;
	mc_language.preview_wait = PREVIEW_WAIT_FR;
	mc_language.preview_close = PREVIEW_CLOSE_FR;
	mc_language.preview_err_no_laser = PREVIEW_ERR_NO_LASER_FR;
	mc_language.preview_err_memory = PREVIEW_ERR_MEMORY_FR;
	mc_language.preview_err_sd = PREVIEW_ERR_SD_FR;
	mc_language.preview_err_open = PREVIEW_ERR_OPEN_FR;
	mc_language.preview_err_render = PREVIEW_ERR_RENDER_FR;
}

void mc_language_init(void) {
	if (mks_grbl.language == French) {
		set_language_pack_fr();
	}
    else {
		set_language_pack_en();
	}
}
    ...
    if((event == LV_EVENT_RELEASED) || (event == LV_EVENT_PRESS_LOST))  {
        ...
        switch(id) {
			...
			case ID_L_DE: 
				...
				MKS_GRBL_CMD_SEND(str_language_fr);
			break;
        }
    }
}
...
void draw_language(void) {
	...
	label_for_btn_name(language_page.imgbtn_de, language_page.label_de, 0, 0, "Francais");
