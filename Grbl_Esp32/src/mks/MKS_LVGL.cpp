...
bool my_indev_touch(struct _lv_indev_drv_t * indev_drv, lv_indev_data_t * data) {
    ...
    boolean touched = tft.getTouch(&rawX, &rawY);
...
void mks_grbl_parg_init(void) {
    ...
    else if(language_select->get() == 2) mks_grbl.language = French;
    ...
    mks_grbl.laser_power_value = (uint16_t)laser_pointer_power->get();
