...
static uint8_t get_event(lv_obj_t* obj) {
	...
	else if (obj == move_page.z_n) 				return ID_M_Z_DOWN;
	else if (obj == move_page.z_p) 				return ID_M_Z_UP;
	...
	else if (obj == move_page.btn_cooling)		return ID_M_COOLING;
	return 0xFF;
}
...
static void set_cooling_btn(void) {
	CoolantState state;
	state = coolant_get_state();
	if(state.Flood == 1) {
		MKS_GRBL_CMD_SEND("M9\n");
		lv_btn_set_style(move_page.btn_cooling, LV_BTN_STYLE_REL, &move_page.btn_color);
		lv_btn_set_style(move_page.btn_cooling, LV_BTN_STYLE_PR, &move_page.btn_color);
	}
	else {
		MKS_GRBL_CMD_SEND("M8\n");
		lv_btn_set_style(move_page.btn_cooling, LV_BTN_STYLE_REL, &move_page.btn_color_press);
		lv_btn_set_style(move_page.btn_cooling, LV_BTN_STYLE_PR, &move_page.btn_color_press);
	}
}
...
void move_ctrl(char axis, uint8_t dir) {
	...
	if(dir == 0) {
		...
		else if(mks_grbl.move_dis == M_100_MM)	step = -100;
	}else {
		if(mks_grbl.move_dis == M_0_1_MM) 			step = 0.1;
		else if(mks_grbl.move_dis == M_1_MM)		step = 1;
		else if(mks_grbl.move_dis == M_10_MM)		step = 10;
	...
	if (is_z_axis) {
		if(mks_grbl.move_speed == LOW_SPEED) 		speed = 200;
		else if(mks_grbl.move_speed == MID_SPEED)	speed = 400;
		else if(mks_grbl.move_speed == HIGHT_SPEED)	speed = 600;
		else speed = 500;
	} else {
		if(mks_grbl.move_speed == LOW_SPEED) 		speed = 800;
		else if(mks_grbl.move_speed == MID_SPEED)	speed = 2000;
		else if(mks_grbl.move_speed == HIGHT_SPEED)	speed = 4000;
		else speed = 1600;
	}
...
void set_home(void) {
	...
	set_click_status(false);
	mks_draw_common_pupup_info("Info", mc_language.dis_homing, " ");
...
static void set_xy_pos(lv_obj_t* obj, lv_event_t event) {
	...
	if(sys.state == State::Idle && mks_get_motor_status() ) {
		...
		mks_draw_common_popup_info_com("Info", mc_language.dis_pos_succeed, " ", event_henadle_pupup_com);
	}else {
		mks_draw_common_popup_info_com("Warning", mc_language.dis_wait_mc_stop, " ", event_henadle_pupup_com);
	}
}
...
static void set_hhome(void) {
	...
	if(hard_limits->get() && homing_enable->get()) {
		...
		mks_draw_common_popup_info_com("Warning", mc_language.dis_no_hard_homing, " ", event_henadle_pupup_com);
	}
}

static void set_cnc_power(bool status, uint16_t power_value) {

	char buf[96];

	if(status) {
		// Disable laser mode temporarily so M3 fires without movement
...
static void set_cnc_ctrl(void) {
	...
	if(mks_grbl.cnc_pwr == GRBL_CNC_OFF) {
		uint16_t power_value = mks_grbl.laser_power_value;
		if (power_value == 0 || power_value > 1000) {
			power_value = 500;
			mks_grbl.laser_power_value = power_value;
		}
		set_cnc_power(true, power_value);
...
void set_step_len(void) {
	if(mks_grbl.move_dis == M_0_1_MM) {
		...
		mks_grbl.move_dis = M_100_MM;
		mks_lv_label_updata(move_page.label_len, "100mm");
	}else if(mks_grbl.move_dis == M_100_MM) {
		mks_grbl.move_dis = M_0_1_MM;
...
static void event_handler(lv_obj_t* obj, lv_event_t event) {
	...
	switch(id) {
		...
		case ID_M_COOLING: set_cooling_btn();  break;
	}
}
...
static void disp_btn(void) {
	...
	move_page.btn_len = mks_lv_btn_set_for_aglin_screen(mks_global.mks_src_3, move_page.btn_len, 110, 42, LV_ALIGN_IN_TOP_LEFT, 10, 8, event_handler);
	move_page.btn_speed = mks_lv_btn_set_for_aglin_screen(mks_global.mks_src_3, move_page.btn_speed, 110, 42, LV_ALIGN_IN_TOP_LEFT, 10, 56, event_handler);
	move_page.btn_spindle = mks_lv_btn_set_for_aglin_screen(mks_global.mks_src_3, move_page.btn_spindle, 110, 42, LV_ALIGN_IN_TOP_LEFT, 10, 104, event_handler);
	move_page.btn_cooling = mks_lv_btn_set_for_aglin_screen(mks_global.mks_src_3, move_page.btn_cooling, 110, 42, LV_ALIGN_IN_TOP_LEFT, 10, 152, event_handler);
...
static void disp_label(void) {
	...
	move_page.label_btn_cooling = mks_lvgl_long_sroll_label_with_wight_set_center(move_page.btn_cooling, move_page.label_btn_cooling, 0, 0, mc_language.cooling,100);
}
